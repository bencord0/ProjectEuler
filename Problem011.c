...
int p_b(int b,int p);
int Problem011(void){return 1;}
int Problem011_(void) {
	int inner, outer;
...
/**
 * Four rounds: up-down, left-right, diag\, diag/.
 *
 */
	...
	for(outer=0;outer<16;outer++) {
		for(inner=0;inner<20;inner++) {
			product = grid[outer][inner] * grid[outer+1][inner]
				* grid[outer+2][inner] * grid[outer+3][inner];
			biggest = (biggest>product)?biggest:product;

			product = grid[inner][outer] * grid[inner][outer+1]
				* grid[inner][outer+2] * grid[inner][outer+3];
			biggest = (biggest>product)?biggest:product;
		}
	}
	...
	for(outer=0;outer<16;outer++) {
		for(inner=0;inner<16;inner++) {
			product = grid[outer][inner] * grid[outer+1][inner+1]
				* grid[outer+2][inner+2] * grid[outer+3][inner+3];

			biggest = (biggest>product)?biggest:product;

			product = grid[outer+3][inner] * grid[outer+2][inner+1]
				* grid[outer+1][inner+2] * grid[outer][inner+3];
